...
def getName(result,file):
    ...
    for module in modules:
        ...
        if not nameResult:
            logger.error("{} is not a valid module name in {} in file {}".format(module,result,file))
            sys.exit(1)
...
def scan(file):
    ...
    if len(exports)>1:
        logger.error("file {} contains more than one module exports, this is currently not supported".format(file))
        sys.exit(1)
    if len(implements)>1:
        logger.error("file {} contains more than one module implementation, this is currently not supported".format(file))
        sys.exit(1)
    #decide file type
    fileType=FileType.Undecided
    if len(exports)>0 and len(implements)>0:
        logger.error("file {} both exports {} and implements {}".format(file,exports[0],implements[0]))
        sys.exit(1)
    if len(exports)==1:
        if len(exports[0])==2:
            ...
            logger.error("file {} contains an export of partitions with an hierarchy with size greater than 2: {}".format(file,exports[0]))
            sys.exit(1)
    if len(implements)==1:
        if len(implements[0])==2:
            ...
            logger.error("file {} contains an implementation of partitions with an hierarchy with size greater than 2: {}".format(file,implements[0]))
            sys.exit(1)
    ...
    if fileType==FileType.ModuleInterfacePartition or fileType==FileType.PrimaryModuleInterface:
        if fileExtension not in ImportableModuleFileExtensions:
            logger.warning("file {} is an importable file but the file extension {} does not match for this kind of file. Should be: {}".format(file,fileExtension,ImportableModuleFileExtensions))
    if fileType==FileType.Moduleimplementation or fileType==FileType.InternalModulePartition:
        if fileExtension not in ModuleImplementationFileExtensions:
            logger.warning("file {} is an implementation file but the file extension {} does not match for this kind of file. Should be: {}".format(file,fileExtension,ModuleImplementationFileExtensions))
    ...
    for element in imports:
        if element[0]=="":
            if moduleName=="":
                logger.error("importing {} in file {} using :partition but the moduleName could not be decided".format(element,file))
                sys.exit(1)
